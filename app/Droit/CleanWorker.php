...
class CleanWorker{
        ...
        foreach ($subscriptions as $item){
            if(!in_array(strtolower($item->email),$emails)) {
                ...
                if($item->activated_at){
                    $this->subscribers['missing'][] = trim($item->email);
                }
                // if not in abos and not confirmed => delete
                if(!$item->activated_at){
                    $this->subscribers['unconfirmed'][] = trim($item->email);
                }
            }
            else{
    ...
    public function emptyAbo()
    {
        $db  = App::make('designpond\newsletter\Newsletter\Repo\NewsletterUserInterface');
        $all = $db->getAll();

        foreach ($all as $abo){
            if($abo->subscriptions->isEmpty()){
                $abo->delete();
            }
        }
    }
